...
const Dashboard = () => {
  ...
  const fetchPatients = () => {
    axios.get(`https://imara-health-backend-v2.onrender.com/getAllUsers?limit=${patientsPerPage}&pages=${currentPage}`)
    .then(response => {
  ...
  const searchPatients = async () => {
    const res = await axios.get(`https://imara-health-backend-v2.onrender.com/searchByUserName?name=${searchText}&limit=${patientsPerPage}&pages=${currentPage}`);
      ...
      <div className="dashboard-patients">
        {patients.map((patient,index) => (
          ...
          <div className="dashboard-patient-right">
            ...
            <button onClick={() => openEditModal(index)} className="edit-button">Edit User Details</button>
